...
{ id: 'viewed', label: 'Recently Viewed', icon: ClockIcon }] as
const;

type SectionId = (typeof sections)[number]['id'];

export function Account() {
  const { user, signOut, lastOrder, wishlist, recentlyViewed, pushToast } = useStore();
  const navigate = useNavigate();
  const [active, setActive] = useState<SectionId>('profile');

  const displayName = user?.name ?? 'Guest';
  const myOrders = lastOrder ? [lastOrder, ...sampleOrders.slice(0, 2)] : sampleOrders.slice(0, 3);
  ...
  return (
    <>
      ...
      <div className="mx-auto grid max-w-page gap-8 px-5 py-10 sm:px-8 lg:grid-cols-[260px_1fr] lg:gap-12 lg:py-14">
        <aside>
          <nav aria-label="Account sections" className="border border-ink/10 bg-white">
            <ul className="divide-y divide-ink/10">
              ...
              <li key={section.id}>
                  ...
                  className={cx(
                    ...
                    <section.icon width={16} height={16} className="text-chestnut" />
                    {section.label}
                  </button>
                </li>
              )}
              <li>
                <button
                  ...
                  <LogOutIcon width={16} height={16} className="text-chestnut" />
                  Logout
                </button>
              </li>
            </ul>
        ...
        <section aria-live="polite" className="min-w-0">
          ...
          <div className="border border-ink/10 bg-white p-7">
              ...
              <div className="mt-7 grid gap-5 sm:grid-cols-2">
                <TextField label="Full Name" defaultValue={user?.name ?? ''} className="sm:col-span-2" />
                <TextField label="Email" type="email" defaultValue={user?.email ?? ''} />
                <TextField label="Phone Number" defaultValue="" inputMode="tel" />
              </div>
              <Button
              className="mt-7"
              onClick={() => pushToast({ title: 'Profile saved.', tone: 'success' })}>
          ...
          <div>
              ...
              <ul className="mt-6 space-y-4">
                {myOrders.map((order) =>
              <li key={order.id} className="border border-ink/10 bg-white p-6">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <p className="label-luxe text-ink/50">{order.id}</p>
                        <p className="mt-2 font-serif text-xl text-ink">{order.status}</p>
                        <p className="mt-1 text-xs text-ink/55">Placed {formatDate(order.placedAt)}</p>
                      </div>
                      <p className="font-serif text-lg text-ink">{formatKsh(order.total)}</p>
                    </div>
                    <ul className="mt-5 flex flex-wrap gap-3">
                      {order.items.map((item) =>
                  <li key={item.name + item.options} className="flex items-center gap-3">
                          <img src={item.image} alt="" className="h-16 w-13 object-cover" loading="lazy" />
                          <span className="text-xs text-ink/60">
                            {item.name}
                            <span className="block text-ink/45">{item.options}</span>
                          </span>
                        </li>
                  )}
                    </ul>
          ...
          <div className="border border-ink/10 bg-white p-7">
              ...
              <p className="mt-2 text-sm text-ink/60">
                Use your order number with the phone number or email on the order.
              </p>
          ...
          <div className="border border-ink/10 bg-white p-7">
              ...
              { label: 'Store Collection', body: 'Mountain Mall, Thika Road, Nairobi' }].
              map((address) =>
              <li key={address.label} className="border border-ink/12 p-5">
                    <p className="label-luxe text-gold">{address.label}</p>
                    <p className="mt-2.5 text-sm text-ink/70">{address.body}</p>
                  </li>
              )}
              </ul>
          ...
          {active === 'payment' &&
          <div className="border border-ink/10 bg-white p-7">
              <h2 className="font-serif text-2xl text-ink">Payment Information</h2>
              <p className="mt-2 text-sm leading-relaxed text-ink/60">
                Available payment methods are configured by Dallian Luxe Hair and shown at checkout. No card
                or M-Pesa details are stored on your profile.
              </p>
